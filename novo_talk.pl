...
parse(Sentence, LF, assertion) :-
	sentenca(LF, nogap, Numero,Genero,Sentence, []).
...
n( author, X^ opd author(X) ).
n( book, X^ opd book(X) ).
n( professor, X^ opd professor(X) ).
n( program, X^ opd program(X) ).
n( programmer, X^ opd programmer(X) ).
n( student, X^ opd  student(X) ).
...
iv( halt, halts, halted,
	halted, halting, X^ opd halt(X) ).

tv( write, writes, wrote,
	written, writing, X^Y^ opd writes(X,Y) ).

tv( meet, meets, met,
	met, meeting, X^Y^ opd meets(X,Y) ).

tv( concern, concerns, concerned,
	concerned, concerning, X^Y^ opd concerns(X,Y) ).

tv( run, runs, ran,
	run, running, X^Y^ opd runs(X,Y) ).
...
sentenca(Carry1,Carry2,Numero,Genero) --> frase(Numero,Genero).
sentenca(Carry1,Carry2,Numero,Genero) --> pergunta(Numero,Genero).
sentenca(Carry1,Carry2,Numero,Genero,Pessoa) --> periodoSimples(Numero,Genero,Pessoa).
sentenca(Carry1,Carry2,Numero,Genero,Pessoa) --> periodoComposto(Numero,Genero,Pessoa).
